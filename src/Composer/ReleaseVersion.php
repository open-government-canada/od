...
namespace Drupal\od\Composer;

use Drupal\od\IniEncoder;
use Composer\Script\Event;
use Symfony\Component\Finder\Finder;
use Symfony\Component\Yaml\Yaml;

/**
 * Updates the version number in OD's component info files.
 */
class ReleaseVersion {
  ...
  public static function execute(Event $event) {
    ...
    // Find all .info files in OD...except in the installed code base.
    $finder = (new Finder())
      ->name('*.info.yml')
      ->in('.')
      ->exclude('html');
  ...
  protected static function updateMakeFile($version) {
    ...
    foreach ($finder as $make_file) {
      ...
      $make['projects']['od']['download']['tag'] = $version;

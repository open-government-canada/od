...
class CommitmentBreadCrumbBuilder extends PathBasedBreadcrumbBuilder {
  ...
  public function applies(RouteMatchInterface $route_match) {
    ...
    if (!empty($parameters['node']) && $parameters['node']->getType() == 'commitment') {
      return TRUE;
    }
    elseif (!empty($path_elements[0]) && $path_elements[0] == 'commitments') {
      return TRUE;
    }
  }
  ...
  public function build(RouteMatchInterface $route_match) {
    ...
    if ($route && !$route->getOption('_admin_route')) {
      ...
      if (!empty($links) && $links[0]->getText() == $this->t('Home')) {
        ...
        $commitments = $this->pathValidator->getUrlIfValid('commitments');
        if (!empty($open_dialogue) && !empty($commitments)) {
          ...
          if (!empty($path_elements[0]) && $path_elements[0] != 'commitments') {

...
class ConsultationBreadCrumbBuilder extends PathBasedBreadcrumbBuilder {
  ...
  public function applies(RouteMatchInterface $route_match) {
    ...
    if (!empty($parameters['node']) && $parameters['node']->getType() == 'consultation') {
      return TRUE;
    }
    elseif (!empty($path_elements[0]) && $path_elements[0] == 'consultations') {
      return TRUE;
    }
  }
  ...
  public function build(RouteMatchInterface $route_match) {
    ...
    if ($route && !$route->getOption('_admin_route')) {
      ...
      if (!empty($links) && $links[0]->getText() == $this->t('Home')) {
        ...
        $consultation = $this->pathValidator->getUrlIfValid('consultations');
        if (!empty($open_dialogue) && !empty($consultation)) {
          ...
          if (!empty($path_elements[0]) && $path_elements[0] != 'consultations') {

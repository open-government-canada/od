 ...
 */
function od_install_tasks() {
  return [
    'od_install_extensions' => [
      'display_name' => t('Install extensions'),
      'display' => TRUE,
      'type' => 'batch',
    ],
    'od_import_language_config' => [
 ...
 */
function od_install_tasks_alter(array &$tasks, array $install_state) {
  ...
  $task = $tasks['od_import_language_config'];
  unset($tasks['od_import_language_config']);
  $tasks = array_merge($tasks, ['od_import_language_config' => $task]);
}
 ...
 */
function od_install_extensions(array &$install_state) {
  $batch = [];
  $modules = [
    'wxt_ext',
    'od_ext',
  ...
  foreach ($modules as $module) {
    $batch['operations'][] = ['od_install_module', (array) $module];
  }
  return $batch;
}
 ...
 */
function od_install_module($module) {
 ...
 *   The current install state.
 */
function od_import_language_config(array &$install_state) {
  ...
  $files = file_scan_directory(drupal_get_path('profile', 'od'), '/.*\.info\.yml/');
